# -*- coding: utf-8 -*-
"""simulador_stick_slip

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1NyY-HYHUdFyf-e0HNJf-7sKdwZYhI9EN
"""
...
parametros = {
    ...
    'second_param_set': {
        ...
        'OD_BHA': 0.2095, 'J_MESA_fixo': 500.0, 'J_BHA_fixo': 394.0, 'G_MOD_fixo': 79.6e9,
    }
}
parametros_comuns = {'XI': 0.02, 'C_MUD_FACTOR': 0.000}
...
def gerar_parametros_sistema(n_gdl, p):
    ...
    for i in range(n_gdl - 1):
        J_equiv = (J_vec[i] * J_vec[i+1]) / (J_vec[i] + J_vec[i+1])
        c_vec[i] = 2 * p['XI'] * np.sqrt(k_vec[i] * J_equiv)
    c_ext_vec = p['C_MUD_FACTOR'] * J_vec
    return {'J': J_vec, 'k': k_vec, 'c': c_vec, 'c_ext': c_ext_vec}
...
def atrito_karnopp(y, WOB, params, p_ativo_global):
    ...
    if np.abs(omega_broca) <= delta:
        return T_transmitido if np.abs(T_transmitido) <= a2*WOB else a2*WOB*np.sign(T_transmitido)
    else:
...
def modelo_Ndof_controlado(t, y, params, WOB, omega_alvo, atrito_func, p_ativo_global, controller):
    ...
    J, k, c, c_ext = params['J'], params['k'], params['c'], params['c_ext']
    ...
    for i in range(n_gdl):
        ...
        dydt_mecanico[2*i+1] = (T_mesa if i==0 else T_esq) - (T_bit if i==n_gdl-1 else T_dir) - c_ext[i]*omega[i]
...
# =============================================================================
def run_simulation(N_GDL, PARAM_SET_ATIVO, MODELO_DE_ATRITO_ATIVO, WOB_operacional, SRPM_operacional, CONTROLLER_TYPE, controller_gains, t_final=100, show_plot=True):
    ...
    y0 = np.zeros(2 * N_GDL + controller.get_num_estados_internos())
    sol = solve_ivp(
        fun=modelo_Ndof_controlado, t_span=[0, t_final], y0=y0, method='RK45', t_eval=t_eval,
...
if __name__ == '__main__':
    run_simulation(
        N_GDL=5, PARAM_SET_ATIVO='second_param_set', MODELO_DE_ATRITO_ATIVO='karnopp',
        WOB_operacional=140e3, SRPM_operacional=100,
        CONTROLLER_TYPE='pi',
        controller_gains={'Kp': 1000.0, 'Ki': 500.0},
        show_plot=True
    )